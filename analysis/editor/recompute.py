...
in outputs/, and saves the session they were computed from alongside them.
"""
...
from analysis import config, detection, smoothness_metrics
...
from analysis.editor.sessions import SESSION_PATH, refresh_seconds, save_session
from analysis.editor.validation import validate_session
...
def recompute(session: dict, trials: dict[str, LoadedTrial], write: bool = True) -> RecomputeResult:
    ...
    Every metric call here is the same function :func:`analysis.pipeline.main` calls, with the same
    ...
    for event in enabled_trunk:
        for label in TRIALS:
            ...
            peak = detection.recompute_trunk_peak(loaded.kin, loaded.fs, start, end) if label == "Novice" else np.nan
    ...
    if write:
        ...
        def write_csv(frame: pd.DataFrame, name: str) -> None:
            if len(frame) == 0:
                ...
                return
            frame.to_csv(config.OUTPUT_DIR / name, index=False)
            written.append(name)
